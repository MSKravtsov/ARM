...
export default function SqueezeSlider({
    value,
    onChange,
    min = 0,
    max = 40,
    ...
    const getColor = (val: number) => {
        if (val <= 10) return { fill: 'bg-emerald-500', border: 'border-emerald-500' };
        if (val <= 20) return { fill: 'bg-amber-500', border: 'border-amber-500' };
        return { fill: 'bg-red-500', border: 'border-red-500' };
    };
    ...
    return (
        <div className="space-y-3">
            ...
            >
                {/* Background Track */}
                <div
                    className="absolute inset-x-0 h-3 bg-gray-200 rounded-full"
                    style={{
                        top: '50%',
            ...
            <div className="flex justify-between text-xs text-slate-500">
                <span>0h</span>
                <span className="font-semibold text-slate-700">{value}h / week</span>
                <span>40h</span>
            </div>
        </div>
    );
